...
export async function generateMetadata({ params }: { params: { locale: string; airline: string; airport: string } }): Promise<Metadata> {
  const locale = localeFromParam(params.locale);
  const t = getTranslations(locale);
  const { departureIata, arrivalIata } = parseFlightSlug(params.airport);
  ...
  const canonicalUrl = generateAirportTerminalCanonicalUrl(airlineCode, params.airport, locale);
  const alternateUrls = generateAlternateUrls(`/airportsterminal/${airlineCode}/${params.airport}`);
...
export default async function AirlineRoutePage({ params }: { params: { locale: string; airline: string; airport: string } }) {
  const locale = localeFromParam(params.locale);
  const { departureIata, arrivalIata } = parseFlightSlug(params.airport);
  // Convert airline slug to airline code (e.g., "indigo" -> "6e", "6e" -> "6e")
  const airlineCode = getAirlineCodeFromSlug(params.airline);
...
{safeReplace(t?.flightPage?.destinationsOverview || '{airlineName} Destinations Overview', { airlineName })}
      ...
      <SchemaOrg data={breadcrumbSchema([
        ...
        { name: 'Airlines', url: locale === 'en' ? '/airportsterminal' : `/${locale}/airportsterminal` },
        { name: airlineName, url: locale === 'en' ? `/airportsterminal/${airlineCode}` : `/${locale}/airportsterminal/${airlineCode}` },
        { name: `${departureCity}${arrivalIata ? ` to ${arrivalCity}` : normalizedFlights.length > 0 ? ` to ${normalizedFlights.slice(0, 2).map(f => f.city).join(', ')}${normalizedFlights.length > 2 ? ` and ${normalizedFlights.length - 2} more` : ''}` : ''}`, url: locale === 'en' ? `/airportsterminal/${airlineCode}/${params.airport}` : `/${locale}/airportsterminal/${airlineCode}/${params.airport}` },
      ])} />
      ...
      <SchemaOrg data={{
        ...
        "sameAs": [
          process.env.NEXT_PUBLIC_FACEBOOK_URL || "https://facebook.com/airlinefrom.com",
          process.env.NEXT_PUBLIC_TWITTER_URL || "https://twitter.com/airlinefrom.com",
          process.env.NEXT_PUBLIC_INSTAGRAM_URL || "https://instagram.com/airlinefrom.com"
        ],
        "address": {
          "@type": "PostalAddress",
          "streetAddress": process.env.NEXT_PUBLIC_ADDRESS || "8th the green suite b",
